...
# def prepare_settings(settings):
#     return {
#         "mean": getattr(settings, "mean", [0.485, 0.456, 0.406]),
#         "std": getattr(settings, "std", [0.229, 0.224, 0.225]),
...
def prepare_settings(settings):
    if isinstance(settings, dict):
        return {
            "mean": settings.get("mean", [0.485, 0.456, 0.406]),
            "std": settings.get("std", [0.229, 0.224, 0.225]),
            "url": settings.get("url", None),
            "input_range": (0, 1),
            "input_space": "RGB",
        }
    else:  # ModelCfg (new timm)
        return {
            "mean": getattr(settings, "mean", [0.485, 0.456, 0.406]),
            "std": getattr(settings, "std", [0.229, 0.224, 0.225]),
            "url": getattr(settings, "url", None),
